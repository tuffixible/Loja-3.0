...
with tab1:
    ...
    with st.form("cadastro_produto", clear_on_submit=True):
        ...
        with col1:
            ...
            categoria = st.selectbox("🏷️ Categoria", ["Tênis", "Roupas", "Acessórios"])
        ...
        for i, variante in enumerate(st.session_state.variantes):
            ...
            with col3:
                st.session_state.variantes[i]["cor"] = st.text_input("Cor", key=f"cor_{i}")
            with col4:
                st.session_state.variantes[i]["tamanho"] = st.text_input("Tamanho", key=f"tamanho_{i}")
            with col5:
                st.session_state.variantes[i]["quantidade"] = st.number_input("Quantidade", min_value=0, key=f"qtd_{i}")

        submitted = st.form_submit_button("Cadastrar Produto")
        if submitted:
            if validate_product_data(nome, preco_custo, preco_venda, 0):
                df = load_data("produtos")
                for variante in st.session_state.variantes:
...
with tab3:
    ...
    if not df.empty:
        ...
        total_items_estoque = df['quantidade'].sum()
...
.product-image:hover {
    ...
    box-shadow: 0 0 30px rgba(255,255,255,0.8), 
                0 0 60px rgba(30,136,229,0.6),
                0 0 90px rgba(30,136,229,0.4);
